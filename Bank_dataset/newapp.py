...
def main():
    ...
    duration = st.slider("Last Contact Duration (Seconds)", 0,1.0,5000)
    campaign = st.slider("Number of Contacts During Campaign", 1,0.1,50)
    pdays = st.slider("Days Since Last Contact", 0,1.0,1000)
    previous = st.slider("Number of Contacts Before This Campaign", 0,0.1,10)
    poutcome = st.selectbox("Outcome of Previous Marketing Campaign", ["Failure", "Nonexistent", "Success"])
    emp_var_rate = st.slider("Employment Variation Rate", -4.0,0.1, 4.0)
    cons_price_idx = st.slider("Consumer Price Index", 92.0, 0.1, 95.0)
    cons_conf_idx = st.slider("Consumer Confidence Index", -60.0, 0.1, -10.0)
    euribor3m = st.slider("Euribor 3 Month Rate", 0.0, 0.1, 6.0)
    nr_employed = st.slider("Number of Employees", 4000,1.0, 5500)
